...
import { createJournalAttachmentUploadUrl, addJournalEntry } from "../actions";
import { COACHING_JOURNAL_BUCKET, MAX_JOURNAL_ATTACHMENT_BYTES } from "@/lib/coaching-journal";
import styles from "../kunder.module.css";

export default function JournalComposer({ customerPhone }: { customerPhone: string }) {
  ...
  async function handleSubmit(e: React.FormEvent) {
    ...
    try {
      ...
      const result = await addJournalEntry(customerPhone, formData);
      if (!result.ok) {
    ...
    <div className={styles.composerWrap}>
      ...
      {file && (
        <div className={styles.attachChip}>
          <span>{file.type.startsWith("video/") ? "Video" : "Bild"}: {file.name}</span>
          <button type="button" onClick={removeFile} aria-label="Ta bort bilaga">
            ✕
          </button>
        </div>
      )}
      <form onSubmit={handleSubmit} className={styles.composerRow}>
        <input
          type="file"
          accept="image/*,video/*"
          ref={fileInputRef}
        ...
        />
        <label htmlFor="journal-attachment" className={styles.attachBtn} title="Bifoga bild eller video">
          +
        </label>
        <textarea
          ref={textRef}
          placeholder="Skriv en journalanteckning..."
          rows={6}
